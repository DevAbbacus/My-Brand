...
if( ! function_exists( 'woodmart_woo_wrapper_start' ) ) {
	function woodmart_woo_wrapper_start() {
		...
		echo '<div class="site-content shop-content-area  col-lg-12' . $content_class . '" role="main">';
	}
}
...
if( ! function_exists( 'woodmart_product_categories_nav' ) ) {
	function woodmart_product_categories_nav() {
		...
		$list_args = array(  
			...
			'hide_empty' => false,
			'order' => 'asc',
			'orderby' => apply_filters( 'woodmart_product_categories_nav_order_by', 'name' ),
		);
...
if( ! function_exists( 'woodmart_show_category_ancestors' )) {
	function woodmart_show_category_ancestors() {
		...
		$list_args['walker']					 = new WOODMART_Walker_Category();
		$list_args['order'] = 'asc';
		$list_args['orderby'] = apply_filters( 'woodmart_product_categories_nav_order_by', 'name' );
...
if( ! function_exists( 'woodmart_get_my_account_menu' ) ) {
	function woodmart_get_my_account_menu() {
		...
		$role_array = array('administrator','matexi_all','matexi_marketing','matexi_construction');
		$role_available = in_array($user_roles[0], $role_array);
			//echo "<pre>";print_r($role_available);exit();
