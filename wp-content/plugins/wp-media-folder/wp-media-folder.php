            ...
            if (!empty($queue_id)) {
                if (is_array($queue_id)) {
                    foreach ($queue_id as $queueID) {
                        call_user_func('\Joomunited\WPMF\JUMainQueue::deleteQueue', $queueID);
                    }
                } else {
                    call_user_func('\Joomunited\WPMF\JUMainQueue::deleteQueue', $queue_id);
                }
            }
        },
        2,
...
{
    $tasks_speed = wpmfGetOption('tasks_speed');
    $status_menu_bar = wpmfGetOption('status_menu_bar');

    // check some features use queue
    $google_settings = get_option('_wpmfAddon_cloud_config');
    $dropbox_settings = get_option('_wpmfAddon_dropbox_config');
    $onedrive_settings = get_option('_wpmfAddon_onedrive_config');
    $onedrive_business_settings = get_option('_wpmfAddon_onedrive_business_config');
    $aws3_settings = get_option('_wpmfAddon_aws3_config');
    $folder_options = get_option('wpmf_queue_options');
    $sync_from_ftp         = get_option('wpmf_option_sync_media');
    $sync_from_library = get_option('wpmf_option_sync_media_external');
    $use_queue = (!empty($sync_from_ftp) || !empty($sync_from_library) || !empty($folder_options['enable_physical_folders']) || !empty($google_settings['connected']) || !empty($dropbox_settings['dropboxToken']) || !empty($onedrive_settings['connected']) || !empty($onedrive_business_settings['connected']) || (!empty($aws3_settings['credentials']) && !empty($aws3_settings['credentials']['key']) && !empty($aws3_settings['credentials']['secret'])) ) ? true : false;
    if ($cron) {
        $use_queue = false;
        $status_menu_bar = 0;
        $tasks_speed = 100;
    ...
    $args = array(
        'use_queue' => $use_queue, // required
        'assets_url' => WPMF_PLUGIN_URL . 'assets/js/queue.js',
        'plugin_name' => esc_html__('WP Media Folder', 'wpmf'),
        'plugin_prefix' => 'wpmf', // required
        'plugin_domain' => 'wpmf', // required
        ...
        'queue_options' => array(
            'status_menu_bar' => (int)$status_menu_bar, // required
