		...
		foreach ( $user_plans as $membership ) {
			...
			if ( $membership->has_credit_management() ) {
				...
				$details['next-credits-update'] = array(
					...
					'value' => apply_filters( 'yith_wcmbs_next_credits_update_date', date( wc_date_format(), $membership->next_credits_update ), $membership->next_credits_update ),
				);
			}
			...
			<div class="yith-wcmbs-my-account-membership-container">
				...
				<div class="yith-wcmbs-membership-details">
					<?php foreach ( $details as $detail_key => $detail ): ?>
						...
						?>
						<div class="yith-wcmbs-membership-detail <?php echo $class; ?>">
							<div class="yith-wcmbs-membership-detail__title"><?php echo $detail['title'] ?></div>
							<div class="yith-wcmbs-membership-detail__value"><?php echo $detail['value'] ?></div>
						</div>
					<?php endforeach; ?>
				</div>
								...
								<?php foreach ( $activities as $activity ) : ?>
									<div class="yith-wcmbs-membership-history-activity">
										<div class="yith-wcmbs-membership-history-activity__date">
											<?php echo $activity->get_formatted_date( true, wc_date_format() ); ?>
										</div>
										<div class="yith-wcmbs-membership-history-activity__note">
											<?php echo $activity->get_i18n_note() ?>
										</div>
									</div>
								<?php endforeach; ?>
							</div>
