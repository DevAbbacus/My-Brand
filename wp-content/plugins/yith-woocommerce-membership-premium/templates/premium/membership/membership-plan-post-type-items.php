...
<?php if ( $items && ( $membership || yith_wcmbs_has_full_access() ) ) : ?>
	<div class="yith-wcmbs-membership-plan-items yith-wcmbs-membership-plan-items--<?php echo esc_attr( $post_type ) ?>"
			...
			data-security="<?php echo wp_create_nonce( "yith-wcmbs-get-plan-{$post_type}-items" ) ?>"
	>
		<div class='yith-wcmbs-membership-plan-items__title'><?php echo $title; ?></div>
			...
			<div class='yith-wcmbs-membership-plan-items__items'>
				<?php foreach ( $items as $item_id ) : ?>
					...
					<div class="yith-wcmbs-membership-plan-items__item">
						<div class="yith-wcmbs-membership-plan-items__item__name">
							<?php echo $title ?>
						</div>
						<?php if ( $availability ): ?>
							<div class="yith-wcmbs-membership-plan-items__item__availability">
								<?php echo $availability ?>
							</div>
						<?php endif; ?>

						<?php if ( $download ): ?>
							<div class="yith-wcmbs-membership-plan-items__item__download">
								<?php echo $download ?>
							</div>
						<?php endif; ?>
					</div>
			...
			<div class='yith-wcmbs-membership-plan-items__footer'>
				<?php if ( 1 !== $page || 1 !== $max_num_pages ) : ?>
					<div class='yith-wcmbs-membership-plan-items__pagination'>
						...
						<span class='yith-wcmbs-membership-plan-items__pagination__paging'><?php
							// translators: 1. the current page number; 2. number of total pages; (eg. 2 of 12)
							echo sprintf( _x( '%1$s of %2$s', 'Pagination', 'yith-woocommerce-membership' ),
