		...
		public function add_filters_by_plan() {
			...
			<select class="filter_by_membership_plan" name="filter_by_membership_plan" style="float: none;">
				...
				<?php foreach ( $plans as $plan ) :
					...
					?>
					<option
							value="<?php echo $plan->get_id() ?>"><?php echo $plan->get_name() . $count ?></option>
				<?php endforeach; ?>
			</select>
			<?php
							...
							<td>
								<table class="yith-wcmbs-admin-table">
									<tr>
										...
										<th><?php _e( 'ID', 'yith-woocommerce-membership' ); ?></th>
									...
									<?php foreach ( $history_plans as $p ) : ?>
										<tr>
											...
											<td>
												<a href="<?php echo get_edit_post_link( $p->id ) ?>"><?php echo '#' . $p->id ?></a>
											</td>
