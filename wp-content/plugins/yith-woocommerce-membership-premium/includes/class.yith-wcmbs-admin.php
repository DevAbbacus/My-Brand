	...
	class YITH_WCMBS_Admin {
		...
		public function custom_columns( $column, $post_id ) {
			if ( 'yith_wcmbs_restrict_access' === $column ) {
				...
				if ( $plans ) {
					...
					$plans_html  = '<br />' . implode( '<br />', $plan_titles );
					echo '<span class="dashicons dashicons-groups tips" data-tip="' . __( 'Included in memberships', 'yith-woocommerce-membership' ) . ': ' . $plans_html . '"></span>';
				}
			}
		}
		...
		public function customize_plugin_fw_field_after_text( $field ) {
			if ( isset( $field['yith-wcmbs-after-text'] ) ) {
				echo $field['yith-wcmbs-after-text'];
			}
		}
