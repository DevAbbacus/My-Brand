...
<?php if ( !empty( $plans ) ) : ?>
    ...
    <div class="">
        ...
        <?php foreach ( $plans as $plan ) : ?>
            <p>
                <input id="yith-wcmbs-rap-<?php echo $loop ?>" type="checkbox" name="_yith_wcmbs_restrict_access_plan[<?php echo $loop ?>]" value="<?php echo $plan->get_id() ?>"
                    <?php checked( true, in_array( $plan->get_id(), (array)$restrict_access_plan ), true ) ?> >
                <label for="yith-wcmbs-rap-<?php echo $loop ?>"><?php echo $plan->get_name() ?></label>
            </p>
            <?php $loop++; ?>
        <?php endforeach; ?>
    </div>
