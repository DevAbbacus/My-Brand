...
<div class="wrap">
    ...
    <a href="<?php echo admin_url('admin.php?page=xperts-crms-color-map&action=add');?>" class="page-title-action">Add New</a>
        ...
        <div id="message" class="updated notice notice-success is-dismissible">
            <p>
                ...
                switch (intval($_REQUEST['msg'])){
                    case 1:
                        _e('Color Mapping saved successfully!','xperts-crms-wc');
                        break;
                    case 2:
                        _e('Color Mapping updated successfully!','xperts-crms-wc');
                        break;
                    case 3:
                        _e('Color Mapping deleted successfully!','xperts-crms-wc');
                        break;
                    default:
                        break;
                }
    ...
    <form id="events-filter" method="get" action="<?php echo admin_url('admin.php'); ?>">
        <input type="hidden" name="page" value="xperts-crms-color-map">
        <?php wp_nonce_field( 'bulk-crms-color-mappings' ); ?>
        <p class="search-box">
    ...
    <table class="wp-list-table widefat fixed striped posts">
        ...
        <tbody>
            <?php foreach ($color_mappings as $color_map): ?>
                <tr id="post-<?php echo $color_map->id;?>" class="iedit author-self level-0 post-<?php echo $color_map->id;?> hentry entry">
                    <th scope="row" class="check-column">
                        <input id="cb-select-<?php echo $color_map->id;?>" type="checkbox" name="post[]" value="<?php echo $color_map->id;?>" />
                    </th>
                    <td class="title column-title has-row-actions column-primary page-title">
                        <strong><a class="row-title" href="#"><?php echo $color_map->crms_color;?></a></strong>
                        <div class="row-actions">
                            <span class="edit"><a href="<?php echo admin_url('admin.php?page=xperts-crms-color-map&action=edit&mapping_id='.$color_map->id);?>"><?php _e('Edit','xperts-crms-wc');?></a> | </span>
                            <span class="trash"><a href="<?php echo esc_url( wp_nonce_url( "admin.php?page=xperts-crms-color-map&action=delete&id=$color_map->id", 'delete-mapping_color_'.$color_map->id ) );?>" class="submitdelete"><?php _e('Delete','xperts-crms-wc');?></a></span>
                        </div>
                    </td>
                    <td class="title column-sport has-row-actions column-primary page-title">
                        <?php echo $this->getColorNames($color_map->color_mappings);?>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
