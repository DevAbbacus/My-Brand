...
class WCMCA_Html
{
	...
	public function common_js()
	{
		...
		<script>
				...
				var wcmca_confirm_delete_message = '<?php esc_attr_e('Address will be deleted, Are you sure?','woocommerce-multiple-customer-addresses'); ?>';
				var wcmca_confirm_duplicate_message = '<?php esc_attr_e('Address will be duplicated, are you sure?','woocommerce-multiple-customer-addresses'); ?>';
				var wcmca_state_string = '<?php esc_attr_e('State','woocommerce-multiple-customer-addresses'); ?>';
				var wcmca_postcode_string = '<?php esc_attr_e('Postcode / ZIP','woocommerce-multiple-customer-addresses'); ?>';
				var wcmca_city_string = '<?php esc_attr_e('City','woocommerce-multiple-customer-addresses'); ?>';
			</script>
			<?php 
	}
	...
	public function render_admin_order_page_additional_addresses_loading_tools()
	{
		...
		wp_enqueue_style('wcmca-backend-edit-order-addresses', WCMCA_PLUGIN_PATH.'/css/backend-order-edit.css');
		 ...
		 $js_settings = array(
				'reload_page_alert_message' => __( 'This action will reload the page. Unsaved changes will be lost, proceed?', 'woocommerce-multiple-customer-addresses' )
			);
	...
	public function render_admin_user_addresses_edit_page($user_id = null)
	{
		...
		?>
			...
			<?php if(isset($user_id)) : 
				...
				<label><?php  _e('First Name', 'woocommerce-multiple-customer-addresses'); ?></label><br /><?php if(isset($customer_extra_data['first_name'])) echo $customer_extra_data['first_name'][0]; ?> <br /><br />
				<label><?php  _e('Last Name', 'woocommerce-multiple-customer-addresses'); ?></label><br /><?php if(isset($customer_extra_data['last_name'])) echo $customer_extra_data['last_name'][0];?> <br/><br />
				<label><?php  _e('Email Address', 'woocommerce-multiple-customer-addresses'); ?></label><br /><?php echo $customer_data->user_email; ?> <br/><br />
				
				<label><?php  _e('Billing First Name', 'woocommerce-multiple-customer-addresses'); ?></label><br /><?php if(isset($customer_extra_data['billing_first_name'])) echo $customer_extra_data['billing_first_name'][0]; ?> <br /><br />
				<label><?php  _e('Billing Last Name', 'woocommerce-multiple-customer-addresses'); ?></label><br /><?php if(isset($customer_extra_data['billing_last_name'])) echo $customer_extra_data['billing_last_name'][0];?> <br/><br />
				<label><?php  _e('Biling Email Address', 'woocommerce-multiple-customer-addresses'); ?></label><br /><?php if(isset($customer_extra_data['billing_email'])) echo $customer_extra_data['billing_email'][0]; ?> <br/><br />
				
				<label><?php  _e('Registration Date', 'woocommerce-multiple-customer-addresses'); ?> </label><br /><?php echo $customer_data->user_registered; ?> <br/><br />
				<label><?php  _e('Roles', 'woocommerce-multiple-customer-addresses'); ?> </label><br /> 
							<?php 
							if ( !empty( $user->roles ) && is_array( $user->roles ) ) {
								foreach ( $user->roles as $role_code )
									echo  $wp_roles->roles[$role_code]["name"]." (". __('Role code:', 'woocommerce-multiple-customer-addresses')." <i>".$role_code."</i>)<br/>";
							} ?> <br/>
				<label><?php _e('More details', 'woocommerce-multiple-customer-addresses'); ?></label><br/>		
				<a class="button" href="<?php echo get_edit_user_link($user_id); ?>" target="_blank" ><?php _e('User page', 'woocommerce-multiple-customer-addresses'); ?></a>
				<a class="button" href="<?php echo get_admin_url(); ?>edit.php?s&post_status=all&post_type=shop_order&action=-1&_customer_user=<?php echo $user_id ?>&filter_action=Filter" target="_blank"><?php _e('Orders list', 'woocommerce-multiple-customer-addresses'); ?></a>
				</div>
	...
	public function render_addresses_list($user_id = null, $type_to_show_in_order_edit_page = null, $include_scripts = true, $is_order_edit = false)
	{
		...
		$user_can_add_new_billing_addresses = /* is_admin() || */ $field_managment_options['max_number_of_billing_addresses'] == 0 ||  count($addresses_by_type['billing']) < $field_managment_options['max_number_of_billing_addresses'];
		$user_can_add_new_shipping_addresses = /* is_admin() ||  */$field_managment_options['max_number_of_shipping_addresses'] == 0 ||  count($addresses_by_type['shipping']) < $field_managment_options['max_number_of_shipping_addresses'];
		...
		<div id="wcmca_custom_addresses">
			...
			<?php if( (current_user_can('manage_woocommerce') && is_admin()) || (!$which_addresses_to_hide['billing'] && !isset($type_to_show_in_order_edit_page) && !$field_managment_options['disable_user_billing_addresses_editing_capabilities']  )): ?>
			...
			<?php endif;
				 if( (current_user_can('manage_woocommerce') && is_admin())|| (!$which_addresses_to_hide['shipping'] && !isset($type_to_show_in_order_edit_page) && !$field_managment_options['disable_user_shipping_addresses_editing_capabilities'] )): ?>
	...
	public function render_add_product_address_for_guest_user($item_cart_id, $cart_item)
	{
		...
		if($wc_product->get_virtual())
			return;
		
		$shipping_per_product_related_options = $wcmca_option_model->shipping_per_product_related_options();
		$default_address_message = __( 'Item will be shipped to the billing address (or to the shipping address if you selected to ship to a different address from the billing).', 'woocommerce-multiple-customer-addresses' );
		$collect_from_store_message = __( 'Pick-up in store.', 'woocommerce-multiple-customer-addresses' );
		...
		<div id="wcmca_actions_container_<?php echo $item_cart_id; ?>">
			<a href="#wcmca_address_form_container_<?php echo $type; ?>" class="button wcmca_add_new_product_address_guest_button wcmca_add_new_address_button" data-cart-item-id="<?php echo $item_cart_id; ?>" ><?php _e('Set address','woocommerce-multiple-customer-addresses'); ?></a>
		...
		<?php if($shipping_per_product_related_options['display_notes_field']): 
				...
				?>
					<label class="wcmca_product_field_label <?php if($shipping_per_product_related_options['is_notes_field_required']) echo ' wcmca_product_required_field_label '; ?>"><?php echo apply_filters('wcmca_product_note_label', __('Note','woocommerce-multiple-customer-addresses')); ?></label>
		...
		if(empty($addresses)): ?>
			...
			<?php else: 
					if($disable_last_used_address): ?>
						<option value="none"><?php _e('Select an address','woocommerce-multiple-customer-addresses'); ?></option>
					<?php else: ?>
	...
	public function create_options_for_item_address_selector($type, $item_cart_id, $addresses, $addresse_type_disabled)
	{
		...
		<option value="<?php echo $item_cart_id."-||-";?>checkout_data-||-last_used_shipping"><?php _e('Use the current shipping address','woocommerce-multiple-customer-addresses'); ?></option>
		<?php if($options['display_pick_up_option']): ?>
			<option value="<?php echo $item_cart_id."-||-";?>checkout_data-||-collect_from_store"><?php _e('Collect from store','woocommerce-multiple-customer-addresses'); ?></option>
		<?php endif;
