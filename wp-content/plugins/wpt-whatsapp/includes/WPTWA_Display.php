...
class WPTWA_Display {
	...
	public function displayButtons () {
		...
		foreach ( $account as $post ) {
			...
			if ( 'round' === strtolower( WPTWA_Utils::getSetting( 'button_style' ) ) ) {
	...
	public function outputHTML () {
		...
		$cache_time = (int) json_decode( WPTWA_Utils::getSetting( 'cache_time' ), true );
		...
		//if ( count( $displayedIds ) > 0 ) {
			...
			echo '<span id="wptwa-config" data-current-language="' . $current_lang . '" data-ids="' . $ids . '" data-page-title="' . get_the_title() . '" data-page-url="' . get_permalink() . '" data-cache-time="' . $cache_time . '"></span>';
		//}
	...
	public function wpEnqueueScripts () {
		...
		wp_localize_script( 'wptwa-public', 'wptwa_ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ) ) );
	}
