...
(function ($) {
  ...
  var $body = $('body'),
    ...
    last_step = yith_wcms.merge_order_info_payment == 1 ? 'order' : 'payment';
  $body_width = function () {
    return $body.width();
  },
    steps = {
    ...
    is_mobile = function () {
      return ($body_width() < yith_wcms.max_mobile_width && yith_wcms.have_mobile_timeline == 1);
    },
    mobile_style = function () {
      if (is_mobile()) {
        $(yith_wcms.dom.checkout_timeline).prependTo($(yith_wcms.dom.checkout_form));
        var checkout_timeline = $(yith_wcms.dom.checkout_timeline);
        $.each(steps, function (step, v) {
          var timeline_step = checkout_timeline.addClass('yith-is-mobile').find(yith_wcms.dom.timeline_id_prefix + step);
          if (timeline_step.length != 0) {
            var content_id = yith_wcms.dom.timeline_id_prefix + step + '-content';
            timeline_step.after('<li class="yith-timeline-step-mobile-content ' + step + '" id="' + content_id.replace('#', '') + '"></li>')
            steps[step].appendTo($(content_id));
          }
        });
        /**
         * Move navigation button
         */
        var active_step = checkout_timeline.find('li.active').data('step');
        $(yith_wcms.dom.timeline_id_prefix + active_step + '-content').append(form_actions);
      }
    };

  $(window).on('yith_wcms_init', mobile_style);
  ...
  var change_step = function (t, current_step, next_step, prev_step) {
    ...
    if (yith_wcms.live_fields_validation == 'yes') {
      ...
      if (allowed_steps && action == 'next') {
        ...
        if (active_step == 'billing' || active_step == 'login') {
          ...
          invalid_field_obj = billing.find(yith_wcms.dom.wc_invalid_required).not('.ywsfd-validate-required');

          if (yith_wcms.extra_brazil_checkout_fields_for_wc.is_enabled == 1) {
            var brazil_billing_extra_field = yith_wcms.extra_brazil_checkout_fields_for_wc.billing;

            if ($(brazil_billing_extra_field.check).val() == brazil_billing_extra_field.value) {

              invalid_field_obj = $.grep(invalid_field_obj, function (value) {
                return $.inArray($(value).attr('id'), brazil_billing_extra_field.skip_to_check) == -1;
              });
            }
          }

          invalid_field = invalid_field_obj.length;

          if (yith_wcms.merge_billing_shipping != 1) {
            invalid_field = check_create_account_fields(invalid_field, create_account, create_account_wrapper);
          }
        }

        //shipping
        if (active_step == 'shipping' || yith_wcms.merge_billing_shipping == 1) {
          ...
          if (yith_wcms.merge_billing_shipping == 1 && shipping_check_exists != 0 && shipping_check.is(':checked') === false) {
            var shipping_under_billing_step = shipping.find(yith_wcms.dom.shipping_under_billing_step);
            invalid_field = invalid_field - shipping_under_billing_step.find(yith_wcms.dom.wc_invalid_required).not('.ywsfd-validate-required').length;
          }
        ...
        if (yith_wcms.is_delivery_date_enabled && active_step == 'order') {
    ...
    if (action == 'next') {
      ...
      if (is_mobile()) {
        steps[current_step].slideUp(yith_wcms.transition_duration, function () {
          steps[next_step].slideDown(yith_wcms.transition_duration, function () {
            /**
             * Move the navigation button under the content on mobile
             */
            $(yith_wcms.dom.timeline_id_prefix + active_step.data('step') + '-content').first().append(form_actions);
          });
          /**
           * Move the navigation button under the content on mobile
           */
      ...
      if (is_mobile()) {
        steps[current_step].slideUp(yith_wcms.transition_duration, function () {
          ...
          steps[prev_step].slideDown(yith_wcms.transition_duration, function () {
            /**
             * Move the navigation button under the content on mobile
             */
            $(yith_wcms.dom.timeline_id_prefix + active_step.data('step') + '-content').append(form_actions);
          });
    ...
    $body.trigger('yith_wcms_step_changed', yith_wcms);
  };
  ...
  $(document).on('blur click change', yith_wcms.dom.billing + ' .validate-required input, ' + yith_wcms.dom.billing + ' .validate-required textarea, ' + yith_wcms.dom.create_account, function (event) {
    ...
    if (invalid_fields != 0 && $(yith_wcms.dom.checkout_timeline).find('.active').data('step') != 'login') {
  ...
  $(document).on('blur click change', yith_wcms.dom.shipping + ' .validate-required input, ' + yith_wcms.dom.shipping + ' .validate-required textarea, ' + yith_wcms.dom.shipping_check + ', ' + yith_wcms.extra_brazil_checkout_fields_for_wc.billing.check, function (event) {
  ...
  var set_cookie_value = function () {
    var form_obj 	= $('.checkout.woocommerce-checkout'),
      form_temp 	= form_obj.serialize(),
      checkbox_not_checked = '';
    form_obj.find(':checkbox:not(:checked)').each(function(){
      checkbox_not_checked = checkbox_not_checked + '&' + $(this).attr('name') + '=0';
    });

    form_temp = form_temp + checkbox_not_checked;
  ...
  var cache_form_value = function () {
    ...
    $(yith_wcms.dom.checkout_form).find(yith_wcms.dom.required_fields_check).add( yith_wcms.dom.shipping_check ).on('blur change', function (e) {
      set_cookie_value();
    });
  };
        ...
        if (typeof input_field != 'undefined') {
          ...
          if (yith_wcms.dom.select2_fields.indexOf(form_value[0]) != -1) {
            ...
            if (form_value[0].indexOf('_wp') == -1 && cached_value && skip_current_value == false) {
              ...
              if (input_field.prop('type') == 'checkbox') {

                input_field.prop('checked', cached_value == 1 ? true : false );
              } else {
                input_field.val(cached_value);
              }
            }
